	...
	switch(USART1_RX_BUF[2]) //判断命令类型
	{
		...
		case CMD_RSSI:          
			...
			else
			{
				...
				{
					if(!devInfoQuery(&USART1_RX_BUF[5],Tagbdaddrlist2,6,Tagbdaddrlistlentemp[1]))
					{		            					
						Tagbdaddrlistlen[1] --;				
						usart3_sendData(gu8statereport2,18);	
						if(Tagbdaddrlistlen[1] == 0)
          ...
          current_query_state[2] = 1;					
			...
			{							
				if(usartx_state[0] == 1 && flush_tag_process_indicate[0] == 0)
				{
					...
					memcpy(gu8DataSend,USART3_RX_BUF,12);		
					uart4_sendData(gu8DataSend,12);
					set_updReg();
					ledflashfreq[0] = 20;
				}
				else if(usartx_state[1] == 1 && flush_tag_process_indicate[1] == 0)
				{
					...
					current_query_state[1] = 1;
					USART3_RX_BUF[11] = 0x00;
					memcpy(gu8DataSend2,USART3_RX_BUF,12);						
					usart1_sendData(gu8DataSend2,12);
					set_updReg2();	
					ledflashfreq[1] = 20;						
				}
				else if(usartx_state[2] == 1 && flush_tag_process_indicate[2] == 0)
				{
					...
					current_query_state[2] = 1;
					USART3_RX_BUF[11] = 0x00;
					memcpy(gu8DataSend3,USART3_RX_BUF,12);				
					usart6_sendData(gu8DataSend3,12);	
					set_updReg3();	
					ledflashfreq[2] = 20;						
				}	
	...
	switch(USART6_RX_BUF[2]) //判断命令类型
	{
		...
		case CMD_RSSI:
			...
			else
			{
				...
				{
					if(!devInfoQuery(&USART6_RX_BUF[5],Tagbdaddrlist3,6,Tagbdaddrlistlentemp[2]))
					{		            					
						Tagbdaddrlistlen[2] --;				
						usart3_sendData(gu8statereport3,18);	
						if(Tagbdaddrlistlen[2] == 0)
